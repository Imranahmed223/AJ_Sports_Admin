...
function Tables() {
  ...
  const { adv, errors, message, loading } = useSelector(
        ...
        <MDBox pt={4} pb={3} px={3}>
          ...
          >
            {(formik) => (
              <Form>
                <FormInput
                  label="Enter Type Of Advertisement"
                  name="type"
                  type="text"
                />
                <FormInput label="Enter Text" name="text" type="text" />
                <FormInput label="Enter Link" name="link" type="text" />
                <FormInput label="Enter Priority" name="priority" type="text" />
                <p>Enter Image</p>
                <input
                  ref={imageInputRef}
                  type="file"
                  ...
                  <tbody>
                    {adv.length > 0 ? (
                      adv.map((data, ind) => {
                        return (
                          <tr key={ind}>
                            <td className="data-label">{data.type}</td>
                            <td className="data-label">{data.text}</td>
                            <td className="data-label">{data.link}</td>
                            <td className="data-label">{data.priority}</td>
                            ...
                            <td>
                              <span className="action-edit-btn">
                                <EditIcon
                                  ...
                                  onClick={() =>
                                    navigate(`/editAdv/${data.id}`)
                                  }
                                />
                              </span>
                              <span className="action-delete-btn">
                                <DeleteIcon
                                  onClick={() => handleDeleteAdv(data.id)}
                                  fontSize="medium"
                                  style={{
                                    cursor: "pointer",
                                  }}
