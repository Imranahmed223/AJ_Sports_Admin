...
function Tables() {
  ...
  const createValidation = Yup.object({
    ...
    born: Yup.date().required("Born is required"),
    age: Yup.number().required("Age is required"),
    country: Yup.string().required("Country is required"),
  });
  const dispatch = useDispatch();
  const { boxingPlayer, totalPages, errors, message, createLoading, loading } =
    useSelector((state) => state.boxingPlayerReducer);
  ...
  useEffect(() => {
    if (boxingPlayer.length <= 0) {
      dispatch(GetAllBoxingPlayer(page));
    }
  }, []);
  ...
  const handleDeleteboxingPlayer = (id) => {
    let value = window.confirm("Are u Sure You Want To Delete This?");
    if (value) {
      dispatch(DeleteboxingPlayer(id));
    }
  };
      ...
      <Card>
        ...
        <MDBox pt={4} pb={3} px={3}>
          <Formik
            ...
            onSubmit={(values, { resetForm }) => {
              ...
              if (image === "") {
                ...
                dispatch(CreateBoxingPlayer(result));
                // resetForm();
          ...
          >
            {(formik) => (
              <Form>
                <FormInput label="Enter Name" name="name" type="text" />
                <FormInput label="Enter Born" name="born" type="date" />
                <FormInput label="Enter Age" name="age" type="number" />
                <FormInput label="Enter Country" name="country" type="text" />
                <p>Enter Logo</p>
                <input
                  ref={imageInputRef}
                  type="file"
              ...
              <div className="table-container">
                <table>
                  ...
                  <tbody>
                    ...
                    ) : boxingPlayer.length > 0 ? (
                      boxingPlayer.map((data, ind) => {
                        return (
                          <tr key={ind}>
                            <td className="data-label">{data.name}</td>
                            <td className="data-label">{data.born}</td>
                            <td className="data-label">{data.age}</td>
                            <td className="data-label">{data.country}</td>
                            <td className="data-label">{data.active}</td>
                            ...
                            <td>
                              <span className="action-edit-btn">
                                <EditIcon
                                  ...
                                  onClick={() =>
                                    navigate(`/edit-boxing-player/${data.id}`)
                                  }
                                />
                              </span>
                              <span className="action-delete-btn">
                                <DeleteIcon
                                  onClick={() =>
                                    handleDeleteboxingPlayer(data.id)
                                  }
                                  fontSize="medium"
                                  style={{
                                    cursor: "pointer",
